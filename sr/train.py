...
# torch >= 2.6 changed torch.load's default `weights_only` to True, which
# breaks loading PL-saved ckpts that embed pickled Lightning callbacks
# (ModelCheckpoint, etc). PL 1.4.2's cloud_io.load doesn't pass weights_only,
# so we monkey-patch the default. Trusted local ckpts only.
try:
    _orig_torch_load = torch.load
    def _torch_load_wo_false(*args, **kwargs):
        kwargs.setdefault('weights_only', False)
        return _orig_torch_load(*args, **kwargs)
    torch.load = _torch_load_wo_false
except Exception:
    pass
...
# Enable TF32 for matmul ops (cuDNN TF32 for convs is on by default).
# Speeds up training/inference and lowers peak memory on Ampere+ GPUs;
# a harmless no-op on older hardware.
try:
    torch.set_float32_matmul_precision('high')
except Exception:
    # Older torch (< 1.12) doesn't have this API; fall back to the flag.
