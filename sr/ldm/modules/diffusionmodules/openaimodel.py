...
class EncoderUNetModelWT(nn.Module):
    ...
    iRAG extension: when `use_ref_branch=True`, forward() additionally accepts
    a reference image `x_ref` (e.g. the intermediate restoration I_inter from
    TTSR). Both x and x_ref are processed by the same (shared) backbone, and
    their per-scale features are channel-concatenated:
        F_cond[size] = concat( eps_phi(x)[size], eps_phi(x_ref)[size] )  (Eq.4)
    This doubles the effective channel dim of the returned condition dict,
    so the downstream U-Net must set `semb_channels = 2 * encoder.out_channels`
    when the ref branch is enabled.
    """
    ...
    def _encode_single(self, x, emb):
        """Shared backbone forward. Returns dict {str(size): feat[N, out_ch, h, w]}."""
        result_list = []
        results = {}
        h = x.type(self.dtype)
        for module in self.input_blocks:
    ...
    def forward(self, x, timesteps, x_ref=None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of LR inputs.
        :param timesteps: a 1-D batch of timesteps.
        :param x_ref: optional [N x C x ...] Tensor (e.g. I_inter from TTSR).
            Used only when self.use_ref_branch is True; processed through the
            same backbone (shared weights) and concatenated per-scale.
        :return: dict {str(size): feat}. Channel dim is `out_channels` when the
            ref branch is off, or `2 * out_channels` when it is on.
        """
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))

        results = self._encode_single(x, emb)

        if self.use_ref_branch:
            assert x_ref is not None, (
                "EncoderUNetModelWT was built with use_ref_branch=True but "
                "x_ref was not provided in forward()."
            )
            results_ref = self._encode_single(x_ref, emb)
            for k in results:
                results[k] = th.cat([results[k], results_ref[k]], dim=1)
