...
def _torch_load_compat(path, map_location="cpu"):
    """torch.load that works across torch versions. Torch >= 2.6 defaults to
    weights_only=True, which rejects ckpts containing pickled objects (e.g.
    Base diffusion ckpts embed a Lightning ModelCheckpoint). We pass
    weights_only=False (these are trusted local ckpts); older torch (< 1.13)
    has no such kwarg, so fall back."""
    try:
...
class LatentDiffusionSRiRAG(LatentDiffusionSRTextWT):
    """iRAG: reference-based SR diffusion (paper Eq. 3-7).

    Adds a reference branch to the SFT condition:
        F_cond[size] = concat( eps_phi(z_LR)[size], eps_phi(z_inter)[size] )

    where z_inter = VAE.encode( I_inter ) and I_inter is the intermediate
    restoration. Requires the underlying `structcond_stage_model` (an
    EncoderUNetModelWT) to be built with `use_ref_branch=True`, and the U-Net
    to use `semb_channels = 2 * encoder.out_channels`.

    Two modes (switched by `use_restoration`):

      * use_restoration=True  -- a `restoration_module` (e.g. TTSR) is held
        inside this LightningModule. Each step it consumes pixel-space
        (LR, ref) and produces I_inter on the fly. The intermediate loss
        `L_inter` is added to the diffusion loss as in Eq. (7):
            L = L_inter + inter_loss_weight * L_final
        (Note: the paper's alpha balances L_final; we keep that convention
        and label the kwarg `inter_loss_weight` accordingly.)

      * use_restoration=False -- the batch is expected to carry a precomputed
        `I_inter` under `inter_key`. The restoration module is NOT held.
        Only L_final is optimized. Use this when TTSR has been trained
        separately and its outputs are cached on disk.

    Dataset contract (online mode):
        batch['gt']  : HR ground truth, pixel-space, [0, 1] (parent's degradation
                       pipeline turns this into LR internally).
        batch[ref_key]: HR reference image, pixel-space, [0, 1], same H, W as gt.

    Dataset contract (precomputed mode):
        batch['gt']     : HR ground truth.
        batch[inter_key]: precomputed I_inter from TTSR, same H, W as gt,
                          pixel-space, [0, 1].
        NOTE: in precomputed mode the LR used by TTSR offline must match the
        LR produced by this module's degradation pipeline. If they differ
        (e.g. random Real-ESRGAN degradations), the cached I_inter will be
        misaligned with the LR seen at training time. The recommended setup
        for precomputed mode is to also cache LR alongside I_inter and use a
        deterministic dataset that bypasses on-the-fly degradation. That
        dataset variant is out of scope for stage 1; for now, precomputed
        mode is best paired with `no_degradation_prob=1.0` in the config.
    """

    def __init__(
        self,
        *args,
        restoration_module_config=None,
        restoration_module_ckpt=None,
        ...
        **kwargs,
        ...
        if use_restoration:
            assert restoration_module_config is not None, (
                "LatentDiffusionSRiRAG(use_restoration=True) requires "
                "`restoration_module_config`."
            )
            self.restoration_module = instantiate_from_config(restoration_module_config)
            # Optionally warm-start TTSR from a pretrained checkpoint (e.g. one
            # of the BSR-pretrained Tier-N TTSR runs). Done AFTER super().__init__
            # so the base ckpt (which has no restoration_module.* keys)
            # cannot clobber it.
            if restoration_module_ckpt is not None:
                self._load_restoration_ckpt(restoration_module_ckpt)
            elif ckpt_path is not None:
                # Merged combined checkpoint: the restoration_module weights are
                # carried inside the main checkpoint. init_from_ckpt ran before
                # the module existed (super().__init__) and skipped them, so we
                # load them here. A plain base ckpt has none and is left as-is.
                self._load_restoration_from_main_ckpt(ckpt_path)
        else:
            self.restoration_module = None

        # Sanity-check encoder is configured for the ref branch.
        if not getattr(self.structcond_stage_model, 'use_ref_branch', False):
            print(
                "[iRAG] WARNING: structcond_stage_model.use_ref_branch is False. "
                "Set use_ref_branch=True in the EncoderUNetModelWT config so the "
                "reference features are actually consumed."
            )

    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        """Shape-aware checkpoint loading.

        The parent loads with strict=False, which tolerates missing/unexpected
        keys but STILL raises on shape mismatches. iRAG doubles the SFT/SPADE
        condition channel (semb_channels 256 -> 512, because eps_phi(z_LR) and
        eps_phi(z_inter) are concatenated), so the SPADE `mlp_shared.0.*` weights
        in the base ckpt no longer match. Those are exactly the new
        ref-aware SFT params we intend to train from scratch, so we drop any
        key whose shape disagrees with the current model and report it.
        """
        sd = _torch_load_compat(path)
        if "state_dict" in list(sd.keys()):
            sd = sd["state_dict"]
        ...
        for k in list(sd.keys()):
            if k in model_sd and sd[k].shape != model_sd[k].shape:
                dropped_shape.append((k, tuple(sd[k].shape), tuple(model_sd[k].shape)))
                del sd[k]
        missing, unexpected = self.load_state_dict(sd, strict=False)
        print('<<<<<<<<<<<<>>>>>>>>>>>>>>>')
        print(f"Restored from {path} with {len(missing)} missing and "
              f"{len(unexpected)} unexpected keys")
        if dropped_shape:
            print(f"[iRAG] dropped {len(dropped_shape)} shape-mismatched keys "
                  f"(expected: ref-aware SFT/SPADE that must be trained). Sample:")
            for k, a, b in dropped_shape[:6]:
                print(f"[iRAG]   {k}: ckpt {a} vs model {b}")

    def _load_restoration_ckpt(self, path):
        """Warm-start the restoration module (TTSR) from a standalone TTSR
        checkpoint produced by the external TTSR codebase.

        That codebase saves the bare TTSR nn.Module, so keys look like
        `MainNet.*`, `LTE.*`, `LTE_copy.*` (optionally with a `module.`
        DataParallel prefix). Our restoration_module wraps TTSRCore as
        `self.core`, so we strip `module.` and load into `restoration_module.core`.
        `LTE_copy` may be absent in the checkpoint (it is a runtime mirror of
        LTE re-synced at each transferal-perceptual forward), so strict=False.
        """
        sd = _torch_load_compat(path)
        if isinstance(sd, dict) and 'state_dict' in sd and not torch.is_tensor(
                next(iter(sd.values()))):
            sd = sd['state_dict']
        ...
        ret = self.restoration_module.core.load_state_dict(cleaned, strict=False)
        # Only LTE_copy.* is expected to be missing.
        real_missing = [k for k in ret.missing_keys if not k.startswith('LTE_copy.')]
        print(f"[iRAG] loaded restoration_module (TTSR) from {path}")
        print(f"[iRAG]   matched={len(cleaned) - len(ret.unexpected_keys)}, "
              f"unexpected={len(ret.unexpected_keys)}, "
              f"missing(LTE_copy)={len(ret.missing_keys) - len(real_missing)}, "
              f"missing(other)={len(real_missing)}")
        if ret.unexpected_keys:
            print(f"[iRAG]   WARNING unexpected keys (sample): {ret.unexpected_keys[:5]}")

    def _load_restoration_from_main_ckpt(self, path):
        """Load restoration_module weights carried inside the main checkpoint.

        A merged combined checkpoint stores the restoration module under the
        `restoration_module.*` prefix. Since init_from_ckpt runs before the
        module is built, we load those keys into it here. A base checkpoint
        without them leaves the module at its random initialization.
        """
        sd = _torch_load_compat(path)
        if isinstance(sd, dict) and 'state_dict' in sd:
            sd = sd['state_dict']
        prefix = 'restoration_module.'
        resto = {k[len(prefix):]: v for k, v in sd.items() if k.startswith(prefix)}
        if not resto:
            return
        ret = self.restoration_module.load_state_dict(resto, strict=False)
        real_missing = [k for k in ret.missing_keys if not k.startswith('core.LTE_copy.')]
        print(f"[iRAG] loaded restoration_module from merged ckpt {path} "
              f"({len(resto)} keys, missing(other)={len(real_missing)})")
        if real_missing:
            print(f"[iRAG]   WARNING real missing keys (sample): {real_missing[:5]}")

    # ------------------------------------------------------------------
    # Helpers
    # ------------------------------------------------------------------
    def _vae_encode(self, pixel_img):
        """Pixel-space [-1, 1] image -> VAE latent (detached), matches parent."""
        posterior = self.encode_first_stage(pixel_img)
        return self.get_first_stage_encoding(posterior).detach()

    def _compute_inter(self, lq_pixel, ref_pixel, lr_pixel=None):
        """Run the restoration module on pixel-space inputs. Returns I_inter
        in pixel space, value range matching the module's output.

        `lr_pixel` is the optional native-resolution LR (HR / scale). When
        provided (recommended), TTSR uses it directly as MainNet input
        instead of synthesizing one via bicubic round-trip of lq_pixel.
        """
        assert self.restoration_module is not None, \
            "_compute_inter called but restoration_module is None"
        return self.restoration_module(lq_pixel, ref_pixel, lr_pixel=lr_pixel)

    # ------------------------------------------------------------------
    # get_input / shared_step / forward
    # ------------------------------------------------------------------
    def get_input(self, batch, k=None, return_first_stage_outputs=False,
        ...
        """Bypass parent's Real-ESRGAN degradation pipeline.

        IRAGPairedDataset already provides paired (gt, lq=sr_bicubic, ...)
        from disk, so we skip the on-the-fly degradation that the parent
        synthesizes from `batch['gt'] + kernels`. Otherwise the contract
        matches the parent: returns (z, text_cond, z_gt), and caches
        `self.lq` / `self.gt` as pixel-space [-1, 1] tensors for the
        restoration module to consume in shared_step.

        Falls back to the parent implementation if the batch looks like a
        Real-ESRGAN one (i.e. provides 'kernel1' but not 'lq'), so the same
        subclass can still train on the original RealESRGANDataset if needed.
        """
        if 'lq' not in batch:
            # Fall back to Real-ESRGAN style degradation (parent behavior).
            return super().get_input(
                batch, k,
                return_first_stage_outputs=return_first_stage_outputs,
                force_c_encode=force_c_encode, cond_key=cond_key,
                return_original_cond=return_original_cond,
                bs=bs, val=val, text_cond=text_cond,
                return_gt=return_gt, resize_lq=resize_lq,
            )

        # Direct path: dataset gave us pre-baked LR (sr_bicubic) and GT.
        gt = batch['gt'].to(self.device).float()
        lq = batch['lq'].to(self.device).float()
        if gt.max() > 1.5:   # uint8-style fallback
            gt = gt / 255.0
        if lq.max() > 1.5:
            lq = lq / 255.0

        # Match parent's normalization to [-1, 1] and shape contracts.
        self.lq = (lq * 2.0 - 1.0).clamp(-1.0, 1.0).contiguous()
        self.gt = (gt * 2.0 - 1.0).clamp(-1.0, 1.0).contiguous()

        # parent's get_input also crops at gt_size; we trust the dataset
        # to have already done that (IRAGPairedDataset.gt_size).

        if bs is not None:
            self.lq = self.lq[:bs]
            self.gt = self.gt[:bs]

        # VAE-encode both, matching the parent's z/z_gt convention.
        z = self.get_first_stage_encoding(self.encode_first_stage(self.lq)).detach()
        z_gt = self.get_first_stage_encoding(self.encode_first_stage(self.gt)).detach()

        # Text conditioning: empty strings unless caller supplies otherwise,
        # mirroring parent behavior. Length-match against the batch.
        while len(text_cond) < z.size(0):
            text_cond.append(text_cond[-1])
        if len(text_cond) > z.size(0):
            text_cond = text_cond[:z.size(0)]
        ...
        if return_first_stage_outputs:
            xrec = self.decode_first_stage(z_gt)
            out.extend([self.lq, self.gt, xrec])
        if return_original_cond:
            out.append(None)
        return out

    def shared_step(self, batch, **kwargs):
        # iRAG get_input above already caches self.lq / self.gt as pixel-space
        # [-1, 1] tensors. We rely on those for downstream restoration-module /
        # VAE-encoding.
        z, c, z_gt = self.get_input(batch, self.first_stage_key)

        if self.use_restoration:
            ref = batch[self.ref_key].to(self.device).float()
            if ref.max() > 1.5:  # tolerate uint8-style inputs
                ref = ref / 255.0
            ref = ref * 2.0 - 1.0  # match self.lq / self.gt normalization

            # Optional native-resolution LR (recommended; avoids bicubic
            # round-trip inside TTSR).
            lr_native = batch.get('lr', None)
            if lr_native is not None:
                lr_native = lr_native.to(self.device).float()
                if lr_native.max() > 1.5:
                    lr_native = lr_native / 255.0
                lr_native = lr_native * 2.0 - 1.0

            lq_pixel = self.lq  # already [-1, 1]
            i_inter_pixel = self._compute_inter(lq_pixel, ref, lr_pixel=lr_native)
            i_inter_pixel = torch.clamp(i_inter_pixel, -1.0, 1.0)
        else:
            assert self.inter_key in batch, (
                f"precomputed mode expects '{self.inter_key}' in batch"
            )
            i_inter_pixel = batch[self.inter_key].to(self.device).float()
            if i_inter_pixel.max() > 1.5:
                i_inter_pixel = i_inter_pixel / 255.0
            i_inter_pixel = i_inter_pixel * 2.0 - 1.0
            i_inter_pixel = torch.clamp(i_inter_pixel, -1.0, 1.0)

        z_inter = self._vae_encode(i_inter_pixel)

        # Stash for p_losses (L_inter needs pixel-space pair in online mode).
        self._cur_i_inter_pixel = i_inter_pixel
        self._cur_gt_pixel = self.gt  # already [-1, 1]

        loss = self(z, c, z_gt, z_inter=z_inter)
        return loss

    def forward(self, x, c, gt, *args, z_inter=None, **kwargs):
        assert z_inter is not None, "LatentDiffusionSRiRAG.forward requires z_inter"

        index = np.random.randint(0, self.num_timesteps, size=x.size(0))
        t = torch.from_numpy(index).to(self.device).long()
        t_ori = torch.tensor([self.ori_timesteps[i] for i in index]).long().to(x.device)
        ...
        struct_input = gt if self.test_gt else x
        struc_c = self.structcond_stage_model(struct_input, t_ori, x_ref=z_inter)

        return self.p_losses(gt, c, struc_c, t, t_ori, x, *args, **kwargs)

    # ------------------------------------------------------------------
    # Loss: add L_inter in online mode
    # ------------------------------------------------------------------
    def p_losses(self, x_start, cond, struct_cond, t, t_ori, z_gt, noise=None):
        loss, loss_dict = super().p_losses(
            x_start, cond, struct_cond, t, t_ori, z_gt, noise=noise
        )
        prefix = 'train' if self.training else 'val'

        if self.use_restoration and self._cur_i_inter_pixel is not None:
            # L_inter = ||I_inter - I_HR||_1 (Eq. 6). GAN term is left to the
            # restoration module's own losses if it provides them.
            l_inter = torch.nn.functional.l1_loss(
                self._cur_i_inter_pixel, self._cur_gt_pixel
            )
            loss_dict[f'{prefix}/loss_inter'] = l_inter
            # Eq. (7): L = L_inter + alpha * L_final. The parent loss returned
            # is L_final; we multiply it by inter_loss_weight (alpha) and add
            # L_inter on top.
            loss = l_inter + self.inter_loss_weight * loss
            loss_dict[f'{prefix}/loss'] = loss

        # Clear stash to avoid holding references between steps.
        self._cur_i_inter_pixel = None
        self._cur_gt_pixel = None
        return loss, loss_dict

    # ------------------------------------------------------------------
    # Optimizer: include restoration_module params in online mode
    # ------------------------------------------------------------------
    def configure_optimizers(self):
        opt = super().configure_optimizers()
        if not self.use_restoration or self.restoration_module is None:
            return opt
        # Parent may return either an Optimizer or a (optimizers, schedulers)
        # tuple/list; handle both.
        if isinstance(opt, (list, tuple)) and len(opt) == 2 and isinstance(opt[0], list):
            optimizers, schedulers = opt
            for o in optimizers:
                o.add_param_group({'params': list(self.restoration_module.parameters())})
            return optimizers, schedulers
        if isinstance(opt, (list, tuple)):
            for o in opt:
                o.add_param_group({'params': list(self.restoration_module.parameters())})
            return opt
        opt.add_param_group({'params': list(self.restoration_module.parameters())})
        return opt

    # ------------------------------------------------------------------
    # Image logging during training (ref-aware DDIM sampling)
    # ------------------------------------------------------------------
    @torch.no_grad()
    def log_images(self, batch, N=4, ddim_steps=50, ddim_eta=0.0, **kwargs):
        """Ref-aware image logging for iRAG.

        Replaces the parent's in-ddpm sampler call (which does NOT carry
        `struct_cond_ref` and immediately asserts at the first checkpoint
        trigger when `use_ref_branch=True`) with the DDIMSampler path that
        the inference script already uses for inference. Same generation
        pipeline = visualization during training matches what inference will
        produce on any saved checkpoint.

        Returns a dict with:
          inputs        : LR (sr_bicubic) at HR size
          gt            : HR ground truth
          recon_lq      : VAE round-trip of LR latent (autoencoder sanity)
          inter         : I_inter (precomp from disk OR TTSR output in online mode)
          ref           : HR reference (online mode only; absent in precomp)
          samples       : DDIM SR output using F_cond = concat(eps_phi(z_lr),
                          eps_phi(z_inter)) -- Eq. 4 of the paper

        Tunables:
          N         : how many images per row in the visualization grid (4)
          ddim_steps: 50 (vs 200 for final inference) -- 4x faster, plenty
                      to spot quality trends during training.
          ddim_eta  : 0 = deterministic DDIM.
        """
        from ldm.models.diffusion.ddim import DDIMSampler

        log = {}
        # get_input caches self.lq / self.gt as [-1, 1] pixel-space tensors,
        # returns (z_lr, text_cond, z_gt). We ignore the text_cond from
        # get_input and supply an empty string list to match training.
        z, _, z_gt = self.get_input(batch, self.first_stage_key)
        N = min(N, self.lq.size(0))

        # ---------- static visualizations ----------
        log['inputs'] = self.lq[:N]
        log['gt'] = self.gt[:N]
        log['recon_lq'] = self.decode_first_stage(z[:N])

        # ---------- build I_inter for the ref branch ----------
        if self.use_restoration:
            ref = batch[self.ref_key].to(self.device).float()
            if ref.max() > 1.5:
                ref = ref / 255.0
            ...
            if lr_native is not None:
                ...
                if lr_native.max() > 1.5:
                    lr_native = lr_native / 255.0
                lr_native = (lr_native * 2.0 - 1.0)[:N]
            i_inter_pixel = self._compute_inter(self.lq[:N], ref, lr_pixel=lr_native)
        else:
            assert self.inter_key in batch, (
                f"precomp mode expects '{self.inter_key}' in batch for log_images"
            )
            i_inter_pixel = batch[self.inter_key].to(self.device).float()
            if i_inter_pixel.max() > 1.5:
                i_inter_pixel = i_inter_pixel / 255.0
            i_inter_pixel = (i_inter_pixel * 2.0 - 1.0)[:N]
        i_inter_pixel = torch.clamp(i_inter_pixel, -1.0, 1.0)
        log['inter'] = i_inter_pixel

        # ---------- DDIM sampling with the ref branch ----------
        z_lr = z[:N]
        z_inter = self._vae_encode(i_inter_pixel)
        semantic_c = self.cond_stage_model([''] * N)

        sampler = DDIMSampler(self)
        sampler.make_schedule(ddim_num_steps=ddim_steps, ddim_eta=ddim_eta,
                              verbose=False)

        # x_T initialization: q_sample(z_lr) at t=999 (base-model convention),
        # matches inference.py.
        noise = torch.randn_like(z_lr)
        t_999 = torch.full((N,), 999, device=z_lr.device, dtype=torch.long)
        x_T = self.q_sample(z_lr, t_999, noise=noise)

        samples, _ = sampler.ddim_sampling_sr_t(
            cond=semantic_c,
            struct_cond=z_lr,
            struct_cond_ref=z_inter,         # the iRAG ref branch (Eq. 4)
            shape=z_lr.shape,
            x_T=x_T,
        )
        log['samples'] = torch.clamp(self.decode_first_stage(samples), -1.0, 1.0)
        return log
