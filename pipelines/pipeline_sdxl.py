...
class StableDiffusionXLControlNetPipeline(
    ...
    def __init__(
        ...
        controlnet_ref: Union[ControlNetModel, List[ControlNetModel], Tuple[ControlNetModel], MultiControlNetModel],
    ...
    def __call__(
        ...
        prompt_ref: Union[str, List[str]] = None,
        prompt_ref_2: Optional[Union[str, List[str]]] = None,
        ...
        **kwargs,
        ...
        controlnet_ref = self.controlnet_ref._orig_mod if is_compiled_module(self.controlnet_ref) else self.controlnet_ref
        ...
        (
            prompt_embeds_ref,
            negative_prompt_embeds_ref,
            pooled_prompt_embeds_ref,
            negative_pooled_prompt_embeds_ref,
        ) = self.encode_prompt(
            prompt_ref,
            prompt_ref_2,
            device,
        ...
        images_ref = image if isinstance(image_ref, list) else [image_ref]
        image_ref = self.prepare_image(
            images=images_ref,
            width=width,
            height=height,
            batch_size=batch_size * num_images_per_prompt,
        ...
        add_text_embeds_ref = pooled_prompt_embeds_ref
        ...
        if batch_size > 1:
            ...
            negative_prompt_embeds_ref = negative_prompt_embeds_ref.repeat(batch_size, 1, 1)
            prompt_embeds_ref = prompt_embeds_ref.repeat(batch_size, 1, 1)
            negative_pooled_prompt_embeds_ref = negative_pooled_prompt_embeds_ref.repeat(batch_size, 1)
            add_text_embeds_ref = add_text_embeds_ref.repeat(batch_size, 1)
        ...
        if self.do_classifier_free_guidance:
            ...
            prompt_embeds_ref = torch.cat([negative_prompt_embeds_ref, prompt_embeds_ref], dim=0)
            add_text_embeds_ref = torch.cat([negative_pooled_prompt_embeds_ref, add_text_embeds_ref], dim=0)
        ...
        prompt_embeds_ref = prompt_embeds_ref.to(device)
        add_text_embeds_ref = add_text_embeds_ref.to(device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                added_cond_kwargs_ref = {"text_embeds": add_text_embeds_ref, "time_ids": add_time_ids}
                ...
                if guess_mode and self.do_classifier_free_guidance:
                    ...
                    control_model_input_ref = latents
                    control_model_input_ref = self.scheduler.scale_model_input(control_model_input_ref, t)
                    controlnet_prompt_embeds_ref = prompt_embeds_ref.chunk(2)[1]
                    controlnet_added_cond_kwargs_ref = {
                        "text_embeds": add_text_embeds_ref.chunk(2)[1],
                        "time_ids": add_time_ids.chunk(2)[1],
                    }
                    ...
                    control_model_input_ref = latent_model_input

                    controlnet_prompt_embeds = prompt_embeds
                    controlnet_prompt_embeds_ref = prompt_embeds_ref

                    controlnet_added_cond_kwargs = added_cond_kwargs
                    controlnet_added_cond_kwargs_ref = added_cond_kwargs_ref
                ...
                if h*w<=tile_size*tile_size: 
                    ...
                    rgbs, down_block_res_samples_ref, mid_block_res_sample_ref = self.controlnet_ref(
                        control_model_input,
                        t,
                        encoder_hidden_states=controlnet_prompt_embeds_ref,
                        controlnet_cond=image_ref,
                        conditioning_scale=cond_scale,
                        guess_mode=guess_mode,
                        added_cond_kwargs=controlnet_added_cond_kwargs_ref,
                        return_dict=False,
                    )
                    ...
                    if guess_mode and self.do_classifier_free_guidance:
                        ...
                        down_block_res_samples_ref = [torch.cat([torch.zeros_like(d), d]) for d in down_block_res_samples_ref]
                    ...
                    noise_pred = self.unet(
                        ...
                        down_block_additional_residuals_ref=down_block_res_samples_ref,
                    ...
                    for row in range(grid_rows):
                        ...
                        for col in range(grid_cols):
                            ...
                            img_tile_ref = image_ref[:, :, input_start_y*8:input_end_y*8, input_start_x*8:input_end_x*8]
                            img_list_ref.append(img_tile_ref)
                            ...
                            if len(input_list) == batch_size or col == grid_cols-1:
                                ...
                                img_list_t_ref = torch.cat(img_list_ref, dim=0)
                                ...
                                _, down_block_res_samples_ref, mid_block_res_sample_ref = self.controlnet_ref(
                                    cond_list_t,
                                    t,
                                    encoder_hidden_states=controlnet_prompt_embeds_ref,
                                    controlnet_cond=img_list_t_ref,
                                    conditioning_scale=cond_scale * ref_scale,
                                    guess_mode=guess_mode,
                                    added_cond_kwargs=controlnet_added_cond_kwargs_ref,
                                    return_dict=False,
                                )
