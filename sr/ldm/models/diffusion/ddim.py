...
class DDIMSampler(object):
    ...
    def sample_sr_t(self,
        ...
        samples, intermediates = self.ddim_sampling_sr_t(conditioning, struct_cond, size,
                                                    struct_cond_ref=struct_cond_ref,
                                                    callback=callback,
                                                    img_callback=img_callback,
                                                    quantize_denoised=quantize_x0,
                                                    mask=mask, x0=x0,
    ...
    def p_sample_ddim_sr_t(self, x, c, struct_c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            struct_c_t = self.model.structcond_stage_model(
                struct_c, t, x_ref=struct_cond_ref
            )
            e_t = self.model.apply_model(x, t, c, struct_c_t)
        else:
            t_in = torch.cat([t] * 2)
            struct_c_doubled = torch.cat([struct_c] * 2)
            struct_c_ref_doubled = (
                torch.cat([struct_cond_ref] * 2) if struct_cond_ref is not None else None
            )
            struct_c_t = self.model.structcond_stage_model(
                struct_c_doubled, t_in, x_ref=struct_c_ref_doubled
            )
            x_in = torch.cat([x] * 2)
            c_in = torch.cat([unconditional_conditioning, c])
            e_t_uncond, e_t = self.model.apply_model(x_in, t_in, c_in, struct_c_t).chunk(2)
            e_t = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond)
    ...
    def p_sample_ddim_sr_tcanvas(self, x, c, struct_c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        for row in range(grid_rows):
            ...
            for col in range(grid_cols):
                ...
                if struct_cond_ref is not None:
                    cond_ref_tile = struct_cond_ref[:, :, input_start_y:input_end_y, input_start_x:input_end_x]
                    cond_ref_list.append(cond_ref_tile)

                if len(input_list) == batch_size or col == grid_cols-1:
                    input_list = torch.cat(input_list, dim=0)
                    cond_list = torch.cat(cond_list, dim=0)
                    cond_ref_batch = (
                        torch.cat(cond_ref_list, dim=0) if struct_cond_ref is not None else None
                    )
                    if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                        struct_c_t = self.model.structcond_stage_model(
                            cond_list, t[:input_list.size(0)], x_ref=cond_ref_batch
                        )
                        ...
                        t_in = torch.cat([t] * 2)
                        cond_ref_doubled = (
                            torch.cat([cond_ref_batch] * 2) if cond_ref_batch is not None else None
                        )
                        struct_c_t = self.model.structcond_stage_model(
                            torch.cat([cond_list] * 2),
                            t_in[:cond_list.size(0)*2],
                            x_ref=cond_ref_doubled,
                        )
